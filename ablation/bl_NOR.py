...
OPTIONS:  
      ...
      -N normalize  whether to normalize features = False  # NEW
"""
...
# Normalizer class for min-max normalization
class Normalizer:
...
def Data(src=[]):
    data_obj = o(it=Data,n=0,rows=[],cols=None,all_rows_for_global_stats=[], normalizer=None)
    for x in src:
        if not data_obj.cols:
            add(x, data_obj)  # This will initialize cols
            if hasattr(data_obj, 'cols') and data_obj.cols:
                data_obj.normalizer = Normalizer(data_obj.cols)
        else:
            add(x, data_obj)
            if data_obj.normalizer:
                data_obj.normalizer.update(x)
    return data_obj
...
def clone(data, src=[]):
    ...
    for x in src:
        add(x, new_data)
        if new_data.normalizer:
            new_data.normalizer.update(x)
    return new_data
...
#--------- --------- --------- --------- --------- --------- ------- -------
# NAIVE BAYES REWRITE STARTS HERE

def calculate_global_category_stats(datas):
    """Calculate global category statistics for categorical features once per experiment"""
    if not datas:
        return
...
def like(row, data, nall=100, nh=2):
    ...
    def _col(v, col):
        ...
        if getattr(col, 'it', None) is Sym:
            ...
            return (col.has.get(v, 0) + the.alpha_cnb) / (
                col.n + the.alpha_cnb * n_categories_for_smoothing + the.x
            )
        sd = getattr(col, 'sd', 0) + the.v
        if sd <= the.x:
            return 1.0 if abs(v - getattr(col, 'mu', 0)) < the.x else the.x
    ...
    prior = (getattr(data, 'n', 0) + the.k) / (nall + the.k * nh + the.x)
    ...
    log_prior = math.log(prior) if prior > the.x else math.log(the.x)
    log_likelihoods_sum = sum(math.log(max(n, the.x)) for n in tmp)

    return log_prior + log_likelihoods_sum
# NAIVE BAYES REWRITE ENDS HERE
...
def eg__nbfew(file, experiment_name="before_changes"):
    ...
    for n_pos in n_pos_values:
        ...
        for i in range(100):
            ...
            # Calculate global category statistics once per experiment
            calculate_global_category_stats([pos_dataset, neg_dataset])
...
if not hasattr(the, 'x'):
    the.x = 1e-30
if not hasattr(the, 'alpha_cnb'):
    the.alpha_cnb = 1.0
if not hasattr(the, 'v'):
    the.v = 1e-9  # Default value for variance smoothing
random.seed(the.rseed)
